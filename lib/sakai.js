...
(function () {
  ...
  /* Where a course's Resources tool lives, as a portal URL, or null when the
   * course has no Resources tool. /direct/site/{id}/pages.json lists every page
   * with its tool placements; the placement id, not the page id, is what the
   * sidebar's own tool links use. Some builds wrap the array, so accept both. */
  function resourcesUrlFromPages(pages, siteId, origin) {
    var list = Array.isArray(pages) ? pages : ((pages && pages.sitepage_collection) || []);
    for (var i = 0; i < list.length; i++) {
      var tools = (list[i] && list[i].tools) || [];
      for (var j = 0; j < tools.length; j++) {
        var t = tools[j];
        if (t && t.toolId === 'sakai.resources' && t.id) {
          return (origin || defaultOrigin()) + '/portal/site/' + encodeURIComponent(siteId) +
            '/tool/' + encodeURIComponent(t.id);
        }
      }
    }
    return null;
  ...
  async function fetchResourcesUrl(siteId, origin) {
    var r = await getJson('/direct/site/' + encodeURIComponent(siteId) + '/pages.json', origin);
    if (r.error) return r;
    return { url: resourcesUrlFromPages(r.data, siteId, origin) };
  }

  globalThis.LUMS = Object.assign(globalThis.LUMS || {}, {
    sakai: {
      resourcesUrlFromPages: resourcesUrlFromPages,
      fetchResourcesUrl: fetchResourcesUrl,
      ORIGINS: ORIGINS,
